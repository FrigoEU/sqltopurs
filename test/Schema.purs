...
-- CRUD definitions
getAllTesttable :: forall eff . Client -> Aff (db :: DB | eff) (Array {id :: UUID, d :: Date, twotz :: DateTime, t :: Time, mt :: Maybe Time, myadt :: Maybe MyADT})
getAllTesttable cl  = (map unwrap) <$> query (Query "select * from testtable" :: Query TesttableRec) [] cl

getOneFromTesttable :: forall eff obj. Client -> {id :: UUID | obj} -> Aff (db :: DB | eff) (Maybe {id :: UUID, d :: Date, twotz :: DateTime, t :: Time, mt :: Maybe Time, myadt :: Maybe MyADT})
getOneFromTesttable cl obj = (map unwrap) <$> queryOne (Query "select * from testtable where id = $1" :: Query TesttableRec) [toSql obj.id] cl

deleteFromTesttable :: forall eff obj. Client -> {id :: UUID | obj} -> Aff (db :: DB | eff) (Maybe {id :: UUID, d :: Date, twotz :: DateTime, t :: Time, mt :: Maybe Time, myadt :: Maybe MyADT})
deleteFromTesttable cl obj = (map unwrap) <$> queryOne (Query "delete from testtable where id = $1 RETURNING *" :: Query TesttableRec) [toSql obj.id] cl

upsertTesttable :: forall eff obj. Client -> {id :: UUID, d :: Date, twotz :: DateTime, t :: Time, mt :: Maybe Time, myadt :: Maybe MyADT | obj} -> Aff (db :: DB | eff) (Maybe {id :: UUID, d :: Date, twotz :: DateTime, t :: Time, mt :: Maybe Time, myadt :: Maybe MyADT})
upsertTesttable cl obj = (map unwrap) <$> queryOne (Query "insert into testtable values ($1,$2,$3,$4,$5,$6) ON CONFLICT DO UPDATE RETURNING *" :: Query TesttableRec) [toSql obj.id, toSql obj.d, toSql obj.twotz, toSql obj.t, toSql obj.mt, toSql obj.myadt] cl
